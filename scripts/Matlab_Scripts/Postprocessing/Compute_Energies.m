...
%
function Kin = Compute_Energies(Controls,Kin,Rates,SemiIndicator)    
...
%        vEeVVib0    = Syst.Molecule(iMol).vEeVVib0 + 0.1456.* ones(1,length(Syst.Molecule(iMol).vEeVVib0));
        ...
        if(SemiIndicator)
            Levelq = Syst.Molecule(iMol).T(Temp.iT).LevelqSemiClass(:);
        else
        ...
        for iStep = 1:Kin.T(Temp.iT).NSteps
            ...
            if strcmp(Syst.Molecule(iMol).KinMthdIn, 'StS')
                ...
                LevelPop(:,1) = Kin.T(Temp.iT).Molecule(iMol).PopOverg(iStep,LevelToBin(:))' .* Levelq;
            end
            PopTot = sum(LevelPop);
