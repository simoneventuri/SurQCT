...
%
function Read_Pops(PathToKinRunFldrQCT,SemiIndicator)     
    ...
    for iMol=1:NMolecules
        ...
        PopFileMat = strcat(PathToKinRunFldrQCT, '/Pop_', Syst.CFDComp(iComp).Name);
        ...
        if isfile(strcat(PopFileMat,'.mat'))
            ...
            PopFile = strcat(PathToKinRunFldrQCT, '/pop_', Syst.CFDComp(iComp).Name, '.dat');
...
%             fprintf('Found %i Steps in the 0-D Solution', Kin.T(Temp.iT).NSteps) 
        ...
        if(SemiIndicator)
            Levelg = Syst.Molecule(iMol).T(Temp.iT).GroupsIn.gSemiClass';
        else
            Levelg = Syst.Molecule(iMol).T(Temp.iT).GroupsIn.g;
        end
        ...
        for iStep = 1:Kin.T(Temp.iT).NSteps
            if strcmp(Syst.Molecule(iMol).KinMthdIn, 'StS')
                Kin.T(Temp.iT).Molecule(iMol).Pop(iStep,:) = Kin.T(Temp.iT).Molecule(iMol).PopOverg(iStep,:) .* Levelg';
            else
