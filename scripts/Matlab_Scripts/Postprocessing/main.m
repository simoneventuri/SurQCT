...
global Input Syst Temp Param Kin Rates OtherSyst OtherRates Error
...
Input.Tasks.Write_RatesAsNetwork.IncludeExch           = false;
...
Input.Tasks.Plot_Populations.tSteps                    = [1e-8, 5e-8, 1e-7, 5e-7] %[1.e-13, 1e-12, 1.e-11, 1e-10, 1e-9, 1e-8]%[8.94e-7]%[7.e-6, 30e-6, 100e-6, 5.e-3];
...
for iT = 1:length(Temp.TranVec)
    ...
    PathToKinRunFldrDNN = strcat(Input.Paths.ToKinMainFldrDNN,'/', Input.DNN.nondimfldr,'RunI',Input.Inel.TestNum,'_E',Input.Exch.TestNum,'_D',Input.Diss.TestNum,'/output_', Syst.NameLong, Input.RunSuffix, '_T', Temp.TNowChar,'K_',Input.Kin.Proc.OverallFlg);
